...
def get_phi_polyD3(m):   # return list of 3D polynomial basis functions phi_j(x0,x1,x2) of degree m
    ...
    for n in range(m+1):            # loop over total degree n=0,1,2,...,m
        for n0 in range(n+1):       # loop over all n0 (and n1:=n-n0) with n0+n1=n
            for n1 in range(n - n0 + 1):        # Iteriere über Potenzen für x1 bis zur verbleibenden Ordnung n-n0
                n2 = n - n0 - n1                # Die verbleibende Ordnung für x2 ist n-n0-n1
                phi.append(lambda x, n0=n0, n1=n1, n2=n2: x[0]**n0 * x[1]**n1 * x[2]**n2)
    return lambda x: np.array([phi_j(x) for phi_j in phi])   # return function generating feature vector phi(x) for input x 
...
def get_phi_poly(d,m):   # return list of all polynomial basis functions for input dimensionality d and polynomial degree m
    ...
    list_ni=[x for x in product([x for x in range(m+1)], repeat=d)]           # !!! REPLACE THIS !!!  --> generate all tuples (n1,...,nd) with n1+...+nd<=m using Cartesian product
    ...
    for n in range(m+1):                      # loop over total degree n=0,1,2,...,m
        ...
        for i in idx_list_n:
            ...
            phi += [lambda x, ni=ni: np.prod([x[e] ** ni[e] for e in range(d)])]           # !!! REPLACE THIS !!!  --> add basis function x1^n1*n2^n2*...*xd^nd corresponding to tuple ni=(n1,...,nd) to list
    return lambda x: np.array([phi_j(x) for phi_j in phi])   # return function generating feature vector phi(x) for input x 
