...
class KernelMLPClassifier(Classifier):
    ...
    def fit(self,X,T):
        ...
        self.Wy=np.dot(T.T, np.linalg.inv(self.hz(self.K)))# !!REPLACE!! weight matrix from hidden to output layer 
    ...
    def predict(self,x):
        ...
        :returns None: dummy 
        """
        z=self.hz(np.dot(self.Wz, x))             # !!REPLACE!! firing rates of hidden layer z Formel Skript Seite 48
