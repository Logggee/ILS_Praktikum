...
def getKNearestNeighbors(x,X,k=1):  # realizes nearest neighbor search of x in database
    ...
    """
    return np.argsort([np.linalg.norm(x - i) for i in X])[:k]
...
class Sim(iv.IVisit_Simulation):
    def __init__(self,name_arg="ivisit_GaussDataGeneration.py"):
        ...
        self.x=[3.0,3.0]                                   # new input vector to be classified (will be defined by clicking on image im_results)
    ...
    def step(self):
        ...
        k=3
        idx_KNN = getKNearestNeighbors(self.x,X,k)           # !!REPLACE!! get indexes of K nearest neighbors
        X_KNN = X[idx_KNN]                                   # nearest neighbor vectors
        T_KNN = T[idx_KNN]                                   # nearest neighbor class labels
        print("Nearest neightbor Vectors: ", X_KNN)
        print("Nearest neightbor Class Label: ", T_KNN)
        ...
        P=getClassProbabilities(T_KNN)                # get class probabilities for input x
        c=classify(P)
        # (iv) write results
        d.str_results="-------------------------------------\n"
        d.str_results+="Input x="+str(self.x)+"\n"
        d.str_results+="Class distribution: P="+ str(P) +"\n"
        d.str_results+="Class decision: c="+str(c)+" with P(c)="+str(P[c])+"\n"
        print("Klassenverteilungs Wahrscheinlichkeiten: ", P)
        print("Höchste Wahrscheinlichkeit hat die Klasse: ", c , "mit ", P[c], "%")
